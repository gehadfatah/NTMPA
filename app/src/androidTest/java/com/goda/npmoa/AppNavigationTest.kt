...
import com.goda.npmoa.util.DataBindingIdlingResource
import com.goda.npmoa.data_layer.local.db.DbRepository
import com.goda.npmoa.presentation_layer.common.espresso_idling.EspressoIdlingResource
...
class AppNavigationTest {
    ...
    private lateinit var tasksRepository: DbRepository
...
/*
    ...
    fun tasksScreen_clickOnDrawerIcon_OpensNavigation() {
        ...
        onView(withId(R.id.nav_view)).check(matches(not(ViewMatchers.isDisplayed())))
...
*/
    ...
    fun taskDetailScreen_doubleUpButton() = runBlocking {
        ...
        onView(  withContentDescription(
          ...
          onView(withId(R.id.edit_task_fab)).check(matches(ViewMatchers.isDisplayed()))
          onView(withId(R.id.task_detail_title_text)).check(matches(ViewMatchers.isDisplayed()))
        // 4. Confirm that if we click Up button a second time, we end up back at the home screen.
        onView(  withContentDescription(
                activityScenario
                        .getToolbarNavigationContentDescription()
        )).perform(click())
        onView(withId(R.id.add_task_fab)).check(matches(ViewMatchers.isDisplayed()))
        onView(withId(R.id.tasks_container_layout)).check(matches(ViewMatchers.isDisplayed()))
